...
type Store = {
    cart: CartItem[], //estodo global (de tipo CartItem [es el tipo q cree en el arch types.ts])
    addToCart: (cartItem: CartItem) => void, //funcion actualiza estado, acepta un parametro de tipo CartItem
...
export const useShoppingCart = create<Store>()((set) => ({
    cart: (() => {
        ...
        const cart = localStorage.getItem('cart'); //obtengo el carrito desd el localStorage
        if (cart) {
